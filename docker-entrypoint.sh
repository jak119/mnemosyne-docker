...
case "$1" in
    sync-serve|sync-server)
        ...
        if [ "$has_db" = "0" ] && [ -n "${MNEMOSYNE_SYNC_DB_PATH:-}" ]; then
            extra_args="$extra_args --db-path $MNEMOSYNE_SYNC_DB_PATH"
        fi
        if [ "$has_auth" = "0" ]; then
    ...
    mcp|sync|sync-init|sync-status|sync-generate-key|export|import|import-hindsight|verify|doctor|diagnose|repair|reindex|stats|sleep|remember|store|recall|search|update|edit|delete|forget|bank|migrate|hygiene|profile|config|backups|backup|restore)
        exec mnemosyne "$@"
        ;;
